 ...
 * "insert" implies that the method inserts attributes or tags that
 * are part of a preexisting tag.
 * "write" implies that the enclosing tag is included in the writing.
 */
...
public class DataSetToXMLStAX {
     ...
     * myself.
     */
    public void createFile(DataSet dataset, File dataDirectory, File outFile, SeismogramFileTypes fileType)
        throws IOException, XMLStreamException{

        StAXFileWriter staxWriter = new StAXFileWriter(outFile);
        writeDataSetWithNSInfo(staxWriter.getStreamWriter(), dataset, dataDirectory, fileType);
        staxWriter.close();
    }
     ...
     */
    public void writeDataSet(XMLStreamWriter writer, DataSet dataset, File directory, SeismogramFileTypes fileType)
     ...
     */
    public void writeDataSetWithNSInfo(XMLStreamWriter writer, DataSet dataset, File directory, SeismogramFileTypes fileType)
        throws XMLStreamException, IOException{
     ...
     */
    public void insertDSInfo(XMLStreamWriter writer, DataSet dataset, File directory, SeismogramFileTypes fileType)
        throws XMLStreamException, IOException{
        ...
        for (int i = 0; i < childDataSets.length; i++) {
            ...
            if (useDataSetRef) {
                writeRef(writer, dataset.getDataSet(childDataSets[i]), childDirectory);
            }
            else {
                writeDataSet(writer, dataset.getDataSet(childDataSets[i]), childDirectory, fileType);
            }
        }
    }

    public void writeURLDataSetSeismogram(XMLStreamWriter writer, URLDataSetSeismogram urlDSS, URL base)
    ...
    public void writeRef(XMLStreamWriter writer, DataSet dataset, File directory)
        ...
        writeRef(writer, directory.getName()+"/"+dsFile.getName(), dataset.getName());
     ...
     *  to be in a subdirectory relative to the current dataset.
     */
    public void writeRef(XMLStreamWriter writer, String datasetURL, String linkTitle)
        throws XMLStreamException {
     ...
     */
    public void writeParameter(XMLStreamWriter writer, String name, Object parameter)
        throws XMLStreamException{
     ...
     * inserts the parameter into the given element.
     */
    public void writeParameter(XMLStreamWriter writer, String name, String typeDef, String typeName, String value)
        throws XMLStreamException{
