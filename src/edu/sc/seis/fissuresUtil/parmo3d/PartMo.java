...
/*
    PartMo Annimation adapted from several Java3D tutorial examples
    plus whatever else I picked up  ... tjo 05/2000
*/
...
/*  Work on the surface */
    ...
    public class Traces {
       ...
       PointArray zPointArray = new PointArray(numberOfPoints, GeometryArray.COORDINATES );
       PointArray ePointArray = new PointArray(numberOfPoints, GeometryArray.COORDINATES );
       PointArray nPointArray = new PointArray(numberOfPoints, GeometryArray.COORDINATES );
       ...
       Shaker3D shaker = new Shaker3D(Shaker3D.SINE, numberOfPoints, backAzimuth, angleOfIncidence);
       shaker.getSeisPointArrays(zPointArray,ePointArray,nPointArray);

/*
       int jj = 0;
       while (jj < numberOfPoints) {
...
*/
       ...
       public void getSingAlongPoints(Point3f zSingArray[], Point3f eSingArray[], Point3f nSingArray[]) {
           ...
           int dim    = zSingArray.length;
           boolean read = zPointArray.getCapability(GeometryArray.ALLOW_COORDINATE_READ);
    ...
    public class MotionVector {
       ...
       LineArray GroundMotion = new LineArray(82, LineArray.COORDINATES | LineArray.COLOR_3);
...
//              System.out.println("Index: " + count + " " + i + " " + point);
	...
	BranchGroup AxesText = new Axis().getTextBG();
...
/*  
    ...
    transformed by the PlaceCoordSystem object.  This keeps
    them facing the viewer, but in the proper location.
*/
...
//           System.out.println("GM " + i + " " + GMpoints[i]);
        ...
        tjoInterpolator gmSinger 
             = new tjoInterpolator(alpha, GMsingAlong, GMaxisOfPos, gknots, GMpoints); 
        gmSinger.setSchedulingBounds(new BoundingSphere());

        tjoInterpolator gmZonlySinger 
             = new tjoInterpolator(alpha, GMzOnly_singAlong, GMaxisOfPos, gknots, GMzOnly); 
        gmZonlySinger.setSchedulingBounds(new BoundingSphere());

        tjoInterpolator gmNonlySinger 
             = new tjoInterpolator(alpha, GMnOnly_singAlong, GMaxisOfPos, gknots, GMnOnly); 
        gmNonlySinger.setSchedulingBounds(new BoundingSphere());

        tjoInterpolator gmEonlySinger 
             = new tjoInterpolator(alpha, GMeOnly_singAlong, GMaxisOfPos, gknots, GMeOnly); 
 ...
 /*       objPlaceAxesText.addChild(AxesText);
   ...
   /* public static void main(String[] args) {
        ...
        if (args.length == 2) {
           ...
           baz = Float.parseFloat(args[0]);
           ang = Float.parseFloat(args[1]);
        }


        Frame frame = new MainFrame(new PartMo(baz,ang), 512, 512);

    } // end of main method of partmo*/
