...
public class EventStart {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // get some defaults
            String propFilename = "event.prop";
            String defaultsFilename = "edu/sc/seis/fissuresUtil/anhinga/event/"
                    + propFilename;
            ...
            try {
                props.load((EventStart.class).getClassLoader()
            ...
            String databaseName = props.getProperty("DatabaseName", "scepp");
            ...
            String quitDatabasePassword = props.getProperty("QuitDatabasePassword",
                                                            "");
            dcConn = DriverManager.getConnection(databaseURL,
                                                 databaseName,
                                                 databasePassword);
            checkQuitConn = DriverManager.getConnection(quitDatabaseURL,
                                                        quitDatabaseName,
            ...
            policy_list[2] = rootPOA.create_implicit_activation_policy(org.omg.PortableServer.ImplicitActivationPolicyValue.NO_IMPLICIT_ACTIVATION);
            policy_list[3] = rootPOA.create_request_processing_policy(org.omg.PortableServer.RequestProcessingPolicyValue.USE_SERVANT_MANAGER);
            policy_list[4] = rootPOA.create_servant_retention_policy(org.omg.PortableServer.ServantRetentionPolicyValue.NON_RETAIN);
            // Create a POA for the EventFinder_impl.
            org.omg.PortableServer.POA finder_poa = rootPOA.create_POA("finder",
                                                                       manager,
                                                                       policy_list);
            ...
            EventLocator_impl eimpl = new EventLocator_impl(jdbcEventAccess,
                                                            2,
                                                            finder_poa);
            ...
            serviceName = System.getProperty("anhinga.eventDC.serverName",
                                             serviceName);
            serviceDNS = System.getProperty("anhinga.eventDC.serverDNS",
                                            serviceDNS);
            ...
            // compatibility
            serviceNameAlt = System.getProperty("anhinga.eventDC.serverNameAlt",
                                                serviceName);
            ...
            quit = new JDBCQuitTable(checkQuitConn, serviceDNS + "/"
                    + serviceName + ":" + edu.iris.Fissures.VERSION.value);
            ...
            try {
                if(orb != null && fissuresNamingService != null) {
                    fissuresNamingService.unbind(serviceDNS,
                                                 "EventDC",
                                                 serviceName);
                    if(serviceNameAlt != null) {
                        fissuresNamingService.unbind(serviceDNS,
    ...
    private static Category logger = Category.getInstance(EventStart.class.getName());
} // EventAcessStart
