...
public class ChannelChooser extends JPanel {
    ...
    public ChannelChooser(ProxyNetworkDC[] netdcgiven, boolean showSites,
        ...
        this(netdcgiven, showSites, showNetworks, configuredNetworks,
                defaultSelectableBand, defaultAutoSelectBand);
    }

    public ChannelChooser(ProxyNetworkDC[] netdcgiven, boolean showSites,
            String[] configuredNetworks, String[] selectableBand,
            String[] autoSelectBand) {
        this(netdcgiven, showSites, true, configuredNetworks, selectableBand,
                autoSelectBand);
    }
    ...
    public ChannelChooser(ProxyNetworkDC[] netdcgiven, boolean showSites,
        ...
        this(netdcgiven, showSites, showNetworks, configuredNetworks,
                selectableBand, autoSelectBand, defaultSelectableOrientations,
    ...
    public ChannelChooser(ProxyNetworkDC[] netdcgiven, boolean showSites,
        ...
        this(netdcgiven, showSites, true, configuredNetworks, selectableBand,
                autoSelectBand, selectableOrientations, autoSelectedOrientation);
    }
    ...
    public void addAvailableStationDataListener(
            AvailableStationDataListener dataListener) {
        if (stationRenderer != null) {
            stationRenderer.addAvailableStationDataListener(dataListener);
        } else {
            // this should probably never be allowed to happen???
            throw new NullPointerException(
                    "StationRenderer is null, so cannot add availableStationDataListener");
        }
    }
    ...
    public void recheckNetworkAvailability() {
        if (stationRenderer != null) {
            ...
            // this should probably never be allowed to happen???
            throw new NullPointerException(
                    "StationRenderer is null, so cannot recheckNetworkAvailability");
        }
    }
    ...
    public void setConfiguredNetworks(String[] configuredNetworks) {
        ...
        System.arraycopy(configuredNetworks, 0, this.configuredNetworks, 0,
    ...
    public void appendNetwork(NetworkAccess net) {
        netIdToNetMap.put(
                NetworkIdUtil.toString(net.get_attributes().get_id()), net);
    ...
    private void initWidgets() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                for (int i = first; i <= last; i++) {
                    ...
                    if (selections.isSelectedIndex(i)) {
                        ...
                        NetworkAccess[] tmparray = new NetworkAccess[1];
                        tmparray[0] = net;
                        StationLoader t = new StationLoader(
                                ChannelChooser.this, tmparray);
        ...
        orientationList.getSelectionModel().setSelectionInterval(
    ...
    protected void fireStationSelectedEvent(ListSelectionEvent e) {
        StationSelectionEvent stationSelectionEvent = new StationSelectionEvent(
    ...
    protected void clearStationsFromThread() {
        try {
            ...
            logger.warn(
                    "Caught exception while clearing stations, will continue. Hope all is well...",
                    e);
            // oh well...
    ...
    public Channel[] getChannels() {
        Channel[] outChannels = (Channel[]) channelMap.values().toArray(
                new Channel[0]);
        return outChannels;
    }
    ...
    public Station[] getSelectedStations(MicroSecondDate when) {
        ...
        for (int i = 0; i < in.length; i++) {
            MicroSecondDate b = new MicroSecondDate(
                    in[i].effective_time.start_time);
            MicroSecondDate e = new MicroSecondDate(
                    in[i].effective_time.end_time);
    ...
    public Channel[] getSelectedChannels(MicroSecondDate when) {
        ...
        for (int staNum = 0; staNum < selectedStations.length; staNum++) {
            ...
            if (!showSites) {
                if (orientationList.getSelectedValue().equals(
                        "INDIVIDUAL_CHANNELS")) {
                    ...
                    bandSearch: for (int bandNum = 0; bandNum < selectedChannelCodes.length; bandNum++) {
                        for (int chanNum = 0; chanNum < staChans.length; chanNum++) {
                            for (int h = 0; h < siteCodeHeuristic.length; h++) {
                                if (staChans[chanNum].my_site.get_code().equals(
                                        siteCodeHeuristic[h])
                                        && staChans[chanNum].get_code().equals(
                                                selectedChannelCodes[bandNum])) {
                ...
                search: for (int i = 0; i < inChannels.length; i++) {
                    for (int j = 0; j < selectedSiteCodes.length; j++) {
                        for (int k = 0; k < selectedChannelCodes.length; k++) {
                            if (inChannels[i].my_site.get_code().equals(
                                    selectedSiteCodes[j])
                                    && inChannels[i].get_code().equals(
                                            selectedChannelCodes[k])) {
    ...
    private Channel[] getMotionVector(Channel[] allChannels, String bandCode) {
        ...
        if (tmpH != null && tmpH.length != 0) {
            ...
            tmpV = BestChannelUtil.getChannel(allChannels, bandCode, "Z",
                    tmpH[0].my_site.get_code(), tmpH[0].get_code().substring(1,
                            2));
    ...
    private static final int[] defaultSelectableOrientations = { BEST_CHANNELS,
            VERTICAL_ONLY, HORIZONTAL_ONLY, INDIVIDUAL_CHANNELS,
            THREE_COMPONENT };
    ...
    class BundleListCellRenderer extends DefaultListCellRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            return super.getListCellRendererComponent(list, useValue, index,
...
public void run() {
            ...
            if (configuredNetworks == null || configuredNetworks.length == 0) {
                ...
                for (int counter = 0; counter < configuredNetworks.length; counter++) {
                    try {
                        ...
                        logger.debug("HPC Getting network for "
                                + configuredNetworks[counter] + " at "
                                + nsNetDC.getServerDNS());
                        NetworkAccess[] nets = netdc.a_finder().retrieve_by_code(
                                configuredNetworks[counter]);
                        logger.debug("Got " + nets.length + " networks for "
                                + configuredNetworks[counter]);
                        for (int subCounter = 0; subCounter < nets.length; subCounter++) {
                            if (nets[subCounter] != null) {
                                ...
                                if (storedNets == null) {
                                    ...
                                    System.arraycopy(storedNets, 0, tmp, 0,
                                            storedNets.length);
    ...
    class ChannelLoader extends Thread {
        ...
        void addChannels(Channel[] chans) {
            ...
            fireChannelSelectionEvent(new ChannelSelectionEvent(
                    getSelectedChannels()));
        }
