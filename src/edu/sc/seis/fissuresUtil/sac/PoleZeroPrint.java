...
public class PoleZeroPrint {
    ...
    public static String printPoleZeroSac(NetworkAccess network, Channel chan)
        ...
        try {
            ...
            return printPoleZeroSac(inst);
        } catch(ChannelNotFound cnf) {
            GlobalExceptionHandler.handle("Error while getting polezero response",
                                          cnf);
            throw cnf;
        }
    }

    public static String printPoleZeroSac(Instrumentation inst) {
    ...
    static void addToTable(TextTable table,
        ...
        for(int cnt = 0; cnt < values.length; cnt++) {
            ...
            if(values[cnt].real != 0 || values[cnt].imaginary != 0) {
                ...
                realImg[0] = formatter.format(real);
                realImg[1] = formatter.format(imag);
