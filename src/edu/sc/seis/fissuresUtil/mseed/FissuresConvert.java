...
public class FissuresConvert {
    ...
    public static DataRecord[] toMSeed(LocalSeismogram seis, int seqStart)
        ...
        if(seis.data.discriminator().equals(TimeSeriesType.TYPE_ENCODED)) {
            ...
            outRecords = toMSeed(eData,
                                 seis.channel_id,
                                 start,
                                 (SamplingImpl)seis.sampling_info,
                                 seqStart);
        } else if(seis.data.discriminator().equals(TimeSeriesType.TYPE_FLOAT)) {
            try {
                // for float, 64 bytes = 4 bytes * 16 samples, so each edata
                // holds 62*16 samples
                EncodedData[] eData = new EncodedData[(int)Math.ceil(seis.num_points
                        * 4.0f / (62 * 64))];
                ...
                for(int i = 0; i < eData.length; i++) {
                    ...
                    if(j == 0) {
                        throw new SeedFormatException("try to put 0 float samples into an encodedData object j="
                                + j
                                + " i="
                                + i
                                + " seis.num_ppoints="
            ...
            // not encoded
            throw new SeedFormatException("Can only handle EncodedData now, type="
                    + seis.data.discriminator().value());
    ...
    public static LinkedList toMSeed(EncodedData[] eData,
        ...
        for(int i = 0; i < eData.length; i++) {
            ...
            if(eData[i].values.length + header.getSize() + b1000.getSize()
                    + b100.getSize() < RECORD_SIZE) {
                // ok to use Blockette100 for sampling
            } else if(eData[i].values.length + header.getSize()
                    + b1000.getSize() < RECORD_SIZE) {
                // will fit without Blockette100
                b100 = null;
            } else {
                throw new SeedFormatException("Can't fit data into record "
                        + (eData[i].values.length + header.getSize()
                                + b1000.getSize() + b100.getSize()) + " "
                        + eData[i].values.length + " "
                        + (header.getSize() + b1000.getSize() + b100.getSize()));
            } // end of else
            ...
            QuantityImpl hertz = sampling_info.getFrequency()
                    .convertTo(UnitImpl.HERTZ);
            if(b100 != null) {
    ...
    public static LocalSeismogramImpl toFissures(DataRecord seed)
        ...
        ChannelId channelId = new ChannelId(new NetworkId(header.getNetworkCode()
                                                                  .trim(),
                                                          new Time(isoTime, -1)),
                                            header.getStationIdentifier()
                                                    .trim(),
                                            header.getLocationIdentifier()
                                                    .trim(),
                                            header.getChannelIdentifier()
                                                    .trim(),
                                            new Time(isoTime, -1));
