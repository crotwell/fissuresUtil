    ...
    private class ParticleMotion implements TimeListener, AmpListener,
        ...
        public void draw(Graphics g, Dimension size) {
            if(visible){
                ...
                double hMin = ae.getAmp(horiz.getDataSetSeismogram()).getMinValue();
                double hMax = ae.getAmp(horiz.getDataSetSeismogram()).getMaxValue();
        ...
        private JComponent generateLabel(){
            JPanel labelPanel = new JPanel(new GridLayout(filterToParMo.keySet().size() + 1, 1));
