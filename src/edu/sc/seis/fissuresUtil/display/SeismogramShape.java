...
public class SeismogramShape implements Shape, NamedPlotter {
    ...
    /**
	 * Draws this <code>SeismogramShape</code>on the supplied graphics after
	 * updating it based on the TimeEvent and AmpEvent
	 *
	 */
    ...
    public void draw(Graphics2D canvas,
					 ...
					 AmpEvent aEvent){
		if(visible){
			draw(canvas, tEvent.getTime(dss), aEvent.getAmp(dss), size);
		}
    }
    ...
    /**
	 * Recalculates the values of this SeismogramShape based on the values
	 * passed
	 *
	 ...
	 */
    private Shape draw(Graphics2D canvas,
					   MicroSecondTimeRange time,
					   UnitRangeImpl amp,
					   Dimension size){
		try{
			if(!size.equals(plotSize) ||
			   !time.getInterval().equals(plotInterval)||
				...
				if(samplesPerPixel < 0){
					//logger.debug("less than zero samples per pixel. not plotting");
					plotAll = true;
					return null;
				}
				if(samplesPerPixel < 1){
					//logger.debug("less than one sample per pixel.  plotting all points");
					points = new int[seisEnd - seisStart][2];
					plotAll(size);
					plotAll = true;
				}else{
					//logger.debug("more than one sample per pixel.  Compressed plot");
					points = new int[2][size.width];
					plotCompress(size.height);
					plotAll = false;
				}
			}else if(samplesPerPixel < 1){
				//logger.debug("plotting all pixels on a drag because less than one sample per pixel");
				getEdgeValues(time, size);
				plotAll(size);
			}else{
				//logger.debug("shifting on a drag plot");
				dragPlot(time, size);
			}
			...
			plotAmp = amp;
			plotSize = size;
			pixelHeightPercentage = 1/(double)plotSize.height;
    ...
    private void plotAll(Dimension size) throws CodecException{
		if(seisEnd <= seis.getNumPoints() && seisStart >= 0 && (startPixel != 0 && endPixel != 0)){
			...
			points = new int[seisEnd - seisStart][2];
			samplesPerPixel =
				(seisEnd - seisStart - 1)/(double)(endPixel - startPixel);
    ...
    private void dragPlot(MicroSecondTimeRange time, Dimension size) throws CodecException{
		...
		//logger.debug("offset size is " + offset + " prior to shift removal");
		offset -= shift;
		//logger.debug("offset size is " + offset + " post shift removal");
		double timePixelPercentage = offset/size.width;
		MicroSecondDate adjustedBeginTime = new MicroSecondDate(time.getBeginTime().getMicroSecondTime() -
																	(long)(timePixelPercentage *
																			   time.getInterval().getValue()));
		MicroSecondDate adjustedEndTime = new MicroSecondDate(time.getEndTime().getMicroSecondTime() -
																  (long)(timePixelPercentage *
																			 time.getInterval().getValue()));
		MicroSecondTimeRange adjustedTimeRange = new MicroSecondTimeRange(adjustedBeginTime,
																		  adjustedEndTime);
    ...
    protected double maxAmp, minAmp, samplesPerPixel, range, pixelHeightPercentage;
	...
	private static Category logger = Category.getInstance(SeismogramShape.class.getName());
}// SeismogramShape
