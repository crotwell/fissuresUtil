...
public abstract class Border extends JComponent{
    ...
    private int side, direction, order, labelTickHeight, labelTickWidth,
    ...
    protected abstract class BorderFormat{
         ...
         * getRange method of implementing classes should use the same units
         * as whatever was used to get this division size
         *
         * If ticks per division = 5 then there will be 1 major tick followed by
         * 4 minor ticks for every division.  If it's 10, there will be 1 major
         * tick followed by 9 minor ticks
         */
        ...
        private void label(String label, float[] nextLabelPoint, Graphics2D g2d,
            ...
            float x, y;
            if(direction == VERTICAL){
                y = nextLabelPoint[1] + (float)(bounds.getHeight()/(double)4);
                int xMod = LABEL_TICK_LENGTH +  2;
                if(side == LEFT) x = nextLabelPoint[0] - xMod - (int)bounds.getWidth();
        ...
        //This value is the first value below the minimum that would fall evenly
        //on a major tick boundary.  Therefore, we're labelling and creating borders
        //that are slightly larger and start before the actual unit ranges passed
    ...
    //The combination of order and side determine this border's types as follows
    private static final int LEFT_ASCENDING = 0, RIGHT_ASCENDING = 1, TOP_ASCENDING = 2, BOTTOM_ASCENDING = 3,
        LEFT_DESCENDING = 4, RIGHT_DESCENDING = 5, TOP_DESCENDING = 6, BOTTOM_DESCENDING = 7;
