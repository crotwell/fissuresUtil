...
public class SeismogramFileRefDB extends AbstractHibernateDB {
    ...
    public URLDataSetSeismogram getDataSetSeismogram(ChannelId chan, CacheEvent event, RequestFilter rf) {
        logger.debug("getDataSetSeismogram: " + getTXID() + "  " + ChannelIdUtil.toStringNoDates(chan) + "  " + event
                + "  " + rf.start_time.date_time + "  " + rf.end_time.date_time);
        PrintIfNotCalledOff delayMessage = new PrintIfNotCalledOff(" long running getDataSetSeismogram: " + getTXID()
                + "  " + ChannelIdUtil.toStringNoDates(chan) + "  " + event + "  " + rf.start_time.date_time + "  "
                + rf.end_time.date_time);
    ...
    private List getMatchingSeismogramsFromRefTek(String seismogramFile,
                                                  ...
                                                  MicroSecondDate endTime) throws IOException, RT130FormatException {
        ...
        for (int i = 0; i < seis.length; i++) {
            ...
            // channel).
            if (seis[i].channel_id.channel_code.equals(chanId.channel_code) && seis[i].getBeginTime().before(endTime)
